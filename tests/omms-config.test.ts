...
describe("omms project config file", () => {
  async function projectMaxMemories(files: Record<string, number>): Promise<number> {
    ...
    const result = await runConfigScenario(
      ...
      return { maxMemories: cfg.CONFIG.maxMemories };
      `
    );
    expect(result.ok).toBe(true);
    return result.maxMemories;
  }

  it("applies .opencode/omms.jsonc", async () => {
    expect(await projectMaxMemories({ "omms.jsonc": 41 })).toBe(41);
  });

  it("still applies the legacy .opencode/opencode-mem.jsonc", async () => {
    expect(await projectMaxMemories({ "opencode-mem.jsonc": 42 })).toBe(42);
  });
